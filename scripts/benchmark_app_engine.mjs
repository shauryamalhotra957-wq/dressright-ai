...
import { AppCircuitBreaker, resilientAppExecute } from '../src/utils/appResilience.mjs';

async function runBenchmark() {
  const iterations = 50_000;
  const cb = new AppCircuitBreaker();
  const latencies = [];

  const mockCacheLookup = async () => {
    let sum = 0;
    for (let i = 0; i < 20; i++) sum += i * 3;
    return sum;
  };
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    await resilientAppExecute(mockCacheLookup, { circuitBreaker: cb });
  ...
  console.log(`=== dressright-ai Performance Benchmark ===`);
